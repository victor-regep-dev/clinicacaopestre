    ...
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "bg-petrol/95 backdrop-blur-md shadow-lg py-1" : "py-2"}`}>
      <div className="wrap flex items-center justify-between gap-5">
        ...
        <ul className="hidden lg:flex gap-7 list-none">
          {NAV.map((n, i) => (
            <li key={n.label}><a href={n.href} className={`text-white/90 font-semibold text-[0.86rem] hover:text-white transition ${i === 0 ? "text-white after:content-[''] after:block after:h-0.5 after:bg-brand-orange after:rounded after:mt-1.5" : ""}`}>{n.label}</a></li>
          ))}
        </ul>
